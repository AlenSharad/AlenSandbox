...
table 50102 "Item Attribute Value Buffer"
{
    Caption = 'Item Attribute Value Buffer';
    Description = 'This table is used to show attributes of Item in Custom API. It should only be used as temporary.';
    ...
    keys
    {
        key(Key1; "Attribute Name", "Inherited-From Key Value")
        {
            Clustered = true;
        }
    ...
    procedure InsertItemAttributeValue(var ItemAttributeValue: Record "Item Attribute Value"; TempItemAttributeValueSelection: Record "Item Attribute Value Buffer" temporary)
    var
        ValDecimal: Decimal;
        ValDate: Date;
    begin
    ...
    var
        TempItemAttributeValueSelection: Record "Item Attribute Value Buffer" temporary;
        AttributeName: Text[250];
    begin
        if IsEmpty() then
            exit;
    ...
    procedure FindAttributeValueFromRecord(var ItemAttributeValue: Record "Item Attribute Value"; ItemAttributeValueSelection: Record "Item Attribute Value Buffer"): Boolean
    var
        ValDecimal: Decimal;
        ValDate: Date;
    begin
    ...
    local procedure OnBeforeFindItemAttributeCaseInsensitive(var ItemAttribute: Record "Item Attribute"; var ItemAttributeValueSelection: Record "Item Attribute Value Buffer")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeIsNotBlankDecimal(var ItemAttributeValueSelection: Record "Item Attribute Value Buffer"; TextValue: Text[250]; var Result: Boolean; var IsHandled: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnInsertRecordOnBeforeItemAttrValueSelectionInsert(var ItemAttributeValueSelection: Record "Item Attribute Value Buffer"; TempItemAttributeValue: Record "Item Attribute Value" temporary)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnPopulateItemAttributeValueOnBeforeInsert(ItemAttributeValueSelection: Record "Item Attribute Value Buffer"; var TempItemAttributeValue: Record "Item Attribute Value" temporary);
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnValidateChangedAttributeOnBeforeValidateValue(ItemAttribute: Record "Item Attribute"; var ItemAttributeValueSelection: Record "Item Attribute Value Buffer")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnInsertItemAttributeValueOnBeforeInsertItemAttributeValue(var ItemAttributeValue: Record "Item Attribute Value"; var TempItemAttributeValueSelection: Record "Item Attribute Value Buffer" temporary)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnFindAttributeValueFromRecordOnBeforeFindAttributeValueFromRecord(var ItemAttributeValue: Record "Item Attribute Value"; ItemAttributeValueSelection: Record "Item Attribute Value Buffer")
    begin
    end;
